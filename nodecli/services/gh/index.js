...
function listRepos(owner, account) {
  ...
  const result = spawn("gh", ["repo", "list", owner, "--limit", "50", "--json", "nameWithOwner,isPrivate,description"], { env });
...
async function run() {
  ...
  const repoIdx = await selectMenu(`Chọn repo (${owner}) — ${repos.length} repo`, [
    ...repos.map((r) => ({
  ...
  while (true) {
    const featureIdx = await selectMenu(`Chọn nghiệp vụ — ${selectedRepo}`, [
      { label: "Secrets — thêm/xem/xóa repo secrets" },
      { label: "Actions — xem / kích hoạt / bật-tắt workflows" },
      // Thêm nghiệp vụ mới ở đây
    ]);

    if (featureIdx === -1) break;

    if (featureIdx === 0) await secrets.run(selectedRepo, account);
    if (featureIdx === 1) await actions.run(selectedRepo, account);
  }
}
